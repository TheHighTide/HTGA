...
#include "header/apploader.h"       // The most important header needed to load files that the program uses
#include "header/statman.h"         // The main header for editing statistics
#include "header/errormsg.h"        // This is required for displaying error messages
#include "header/artviewer.h"       // This is for viewing art in the console
#include "header/randomize.h"       // This is for random number or word generation
...
int main(){
    ...
    LoadFileToData("data/timeslaunched.userdata", false, "0", true);
    ...
    while(true){ // The main loop for the appliation
        ...
        if(userInput == "help"){ // The command responsible for displaying all the commands
            cout << "Here is a list of available commands:\n"
                 ...
                 << "time               Displays the current date and time info\n"
            ...
            auto end = std::chrono::system_clock::now(); // Gets how long the program has been running
            std::time_t end_time = std::chrono::system_clock::to_time_t(end); // Gets the current date and time
            chrono::duration<double> elapsed_seconds = end-start; // Calculates how long the program has been running
            cout << "Date & Time: " << ctime(&end_time) << "Runtime: " << elapsed_seconds.count() << " seconds\n"; // Displays the data
        }
